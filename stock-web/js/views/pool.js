            ...
            <tbody>
              ${rows.map(r => `
                <tr>
                  ...
                  <td style="text-align:left;color:#8b949e;font-size:12.5px">${App.esc(r.note || '')}</td>
                </tr>`).join('')}
            </tbody>
          </table>
        </div>
        <p style="font-size:12px;color:#8b949e;margin-top:8px">买点/止损/目标为复盘时框架参考值，非交易指令。状态更新需在本地数据文件中维护（观察中→已买入→已卖出/已放弃）。</p>
      </div>` : '<div class="card"><h2>该日暂无候选池</h2></div>';

    const poolMeta = data && data.note ? `<div class="card" style="font-size:13px;color:#8b949e">${App.esc(data.note)}</div>` : '';

...
const App = {
  ...
  /* Markdown 简化渲染：**加粗** / 行首>引用 / 列表 / 表格（基础） */
  mdToHtml(md) {
    if (!md) return '';
    const bold = s => this.bold(s);   // 内部引用，避免裸调用 ReferenceError
    ...
    for (let line of lines) {
      ...
      if (raw.startsWith('|') && raw.endsWith('|')) {
        ...
        if (!inTable) {
          ...
          cells.forEach(c => { html += `<th>${bold(esc(c))}</th>`; });
          html += '</tr></thead><tbody>';
          inTable = true;
        } else {
          html += '<tr>';
          cells.forEach((c, i) => {
            const cls = i === 0 ? 'name' : '';
            html += `<td class="${cls}">${bold(esc(c))}</td>`;
          });
          html += '</tr>';
        }
        continue;
      ...
      if (raw.startsWith('>')) {
        html += `<div class="review-blockquote">${bold(esc(raw.replace(/^>\s?/, '')))}</div>`;
        continue;
      }
      if (/^[-*]\s/.test(raw)) {
        html += `<p style="margin-left:14px">• ${bold(esc(raw.replace(/^[-*]\s/, '')))}</p>`;
        continue;
      }
      if (/^\d+\.\s/.test(raw)) {
        html += `<p style="margin-left:14px">${bold(esc(raw))}</p>`;
        continue;
      }
      // 独立成行的 **加粗** → 段内小标题（层级：段标题 > 小标题 > 正文）
      if (/^\*\*[^*]+\*\*[：:]?$/.test(raw)) {
        html += `<h4 class="sec-sub">${bold(esc(raw))}</h4>`;
        continue;
      }
      if (/^#{1,4}\s/.test(raw)) {
        const level = raw.match(/^#+/)[0].length;
        const tag = level <= 2 ? 'h3' : 'h4';
        html += `<${tag}>${bold(esc(raw.replace(/^#+\s/, '')))}</${tag}>`;
        continue;
      }
      html += `<p>${bold(esc(raw))}</p>`;
    }
    closeTable();
    return html;
  },
  ...
  /* 轻量 Markdown 加粗 + 中国股市红涨绿跌着色 */
  bold(s) {
    let t = String(s == null ? '' : s);

    // ── 语义着色（先于加粗转换，作用于 ** 包裹的文本）──
    // 涨停 N 家 → N 红色
    t = t.replace(/涨停\s*\*\*?(\d+)\*\*?/g, '涨停 **<span class="up">$1</span>**');
    // 跌停 N 家 → N 绿色
    t = t.replace(/跌停\s*\*\*?(\d+)\*\*?/g, '跌停 **<span class="down">$1</span>**');
    // 最高/高度 N 板 → N 红色
    t = t.replace(/(最高板|高度板|最高)\s*\*\*?(\d+)\s*板\*\*?/g, '$1 **<span class="up">$2 板</span>**');
    // 情绪档位 / 共振判定 → 档位色
    t = t.replace(/(情绪档位|共振判定|涨跌家数比)[：:]?\s*\*\*?([^*：:，。]+)\*\*?/g, (m, pre, val) => {
      let cls = 'flat';
      const v = val.trim();
      if (['冰点', '退潮'].includes(v)) cls = 'down';
      else if (['修复', '亢奋', '共振', '可做'].includes(v)) cls = 'up';
      else if (v.includes('背离')) cls = 'down';
      else if (v.includes('共振')) cls = 'up';
      return `${pre}：**<span class="${cls}">${v}</span>**`;
    });

    // ── 加粗转换 ──
    t = t.replace(/\*\*([^*]+)\*\*/g, '<b>$1</b>').replace(/\*([^*]+)\*/g, '<i>$1</i>');

    // ── 正负数字着色（红正绿负；排除日期 2026-08-21、代码 002412 等）──
    t = t
      .replace(/(?<![0-9])[+]([0-9]+(?:\.[0-9]+)?)(%|亿|万|元)?/g, '<span class="up">+$1$2</span>')
      .replace(/(?<![0-9])-([0-9]+(?:\.[0-9]+)?)(%|亿|万|元)?/g, '<span class="down">-$1$2</span>');
    return t;
  },

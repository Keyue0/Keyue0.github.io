...
const BloggerView = {
  ...
  async render(el) {
    ...
    const empty = !items.length ? `
      <div class="card">
        <h2>暂无博主观点</h2>
        <p style="font-size:14px;color:var(--muted)">本板块用于收录从<strong>抖音 / 小红书</strong>等公开渠道获取的财经博主观点，整理后供复盘参考（非投资建议）。</p>
        <p style="font-size:13.5px;color:var(--muted);margin-top:8px">待录入格式：</p>
        <pre style="background:var(--canvas);border:1px solid var(--hairline);border-radius:8px;padding:12px;font-size:12.5px;color:var(--muted);overflow-x:auto;margin-top:6px">{
  ...
  "link": "https://…"            // 来源链接（可选）
    ...
    el.querySelectorAll('[data-platform]').forEach(b => b.addEventListener('click', () => {
      ...
      listEl.innerHTML = filtered.map(i => this.card(i)).join('') || '<div class="card"><p style="color:var(--muted)">该平台暂无观点</p></div>';
    }));

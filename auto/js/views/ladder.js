  ...
  async render(el, params) {
    ...
    const yestHtml = yest ? `
      <div class="card">
        ...
        ${yest.note ? `<p style="font-size:12.5px;color:var(--muted);margin-top:6px">${App.esc(yest.note)}</p>` : ''}
      </div>` : '';
    ...
    el.innerHTML = `
      ...
      ${hlHtml}
      ${yestHtml}
      ${data && data.note ? `<div class="card" style="font-size:13px;color:var(--muted)">${App.esc(data.note)}</div>` : ''}`;
